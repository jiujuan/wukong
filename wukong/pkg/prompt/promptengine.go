	...
	return []*Template{
		{
			...
			Messages: []MessageTemplate{
				...
				{Role: "user", Content: "你是任务执行Worker，请严格执行子任务并输出可直接使用的结果。\n子任务ID: {{sub_task_id}}\n主任务ID: {{task_id}}\nAction: {{action}}\nParams(JSON): {{params_json}}\n要求：\n1. 结果要与Action对应\n2. 输出使用中文\n3. 内容尽量结构化\n4. 不要解释系统实现细节"},
			},
		},
		{
			...
			Messages: []MessageTemplate{
				...
				{Role: "user", Content: "请以web_search执行器模式处理该任务。\n子任务ID: {{sub_task_id}}\n主任务ID: {{task_id}}\n查询: {{query}}\n参数: {{params_json}}\n输出要求：\n1. 返回3-5条关键信息\n2. 每条包含标题、要点、可信度评估\n3. 最后给出综合结论"},
			},
		},
		{
			...
			Messages: []MessageTemplate{
				...
				{Role: "user", Content: "请以report_gen执行器模式处理该任务。\n子任务ID: {{sub_task_id}}\n主任务ID: {{task_id}}\n报告主题: {{topic}}\n参数: {{params_json}}\n输出要求：\n1. 包含摘要、背景、分析、建议、结论\n2. 结构化分节输出\n3. 适合直接交付"},
			},
		},
		{
			...
			Version:     "v1",
			Messages: []MessageTemplate{
				{Role: "system", Content: "你是ReAct执行引擎。必须只输出JSON，不要输出其他文本。JSON格式为: {\"thought\":\"...\",\"action\":\"tool|final\",\"tool_name\":\"...\",\"tool_params\":{},\"final_answer\":\"...\"}。当 action=tool 时必须给出 tool_name 和 tool_params；当 action=final 时给出 final_answer。当前 skill={{skill_name}}，允许工具白名单={{allowed_tools_json}}"},
		...
		},
		{
			...
			Version:     "v1",
			Messages: []MessageTemplate{
				{Role: "system", Content: "你是任务规划器。把用户任务拆解为可执行DAG子任务。只输出JSON，不要输出任何解释。\nJSON格式: {\"thought\":\"一句整体规划思路\",\"steps\":[{\"id\":\"s1\",\"action\":\"web_search\",\"params\":{},\"depends_on\":[],\"thought\":\"该步骤思路\"}]}\n要求：1) action 必须是简短可执行动作名 2) depends_on 引用步骤id\n3) steps 至少1个，最多8个 4) 保证DAG无环"},
